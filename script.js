// ═══════════════════════════════════════════════════════════════
// LocalizaBus — script.js limpo
// Fluxo com GPS + Photon + OTP Planner + mapa
// ═══════════════════════════════════════════════════════════════
            ...
            <div class="route-step">
                ...
                <div class="step-content">
                    <div class="step-title">
                        ${isWalk ? "Caminhada" : "Ônibus " + (leg.routeShortName || leg.route?.shortName || "—")}
                    </div>
                    <div class="step-meta">
                        ${isWalk
                            ? `Ande ${Math.round(leg.distance)}m (${Math.round(leg.duration / 60)} min)`
                            : `Pegue na parada ${leg.from?.name || "N/A"} sentido ${leg.headsign || "N/A"}`}
                    </div>
                    ${!isWalk ? `<div class="realtime-status" id="rt-${sanitizeId(leg.tripId || leg.routeShortName || "bus")}">Buscando GPS...</div>` : ""}
                </div>
            </div>
        `;
